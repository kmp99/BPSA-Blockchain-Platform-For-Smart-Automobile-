	...
	serviceRequested.watch(function(error, resultRequested){
		...
		if(resultRequested){
          //Store the service request object into json array
			...
			console.log('resultRequested',resultRequested);
			var hasValue;
		 if(sessionStorage.getItem("contracts") != undefined){

		 	recentServiceRequests = JSON.parse(sessionStorage.getItem("contracts"));
			hasValue = recentServiceRequests[recentServiceRequests.length-1].args.remarks == resultRequested.args.remarks;

			// hasValue =	recentServiceRequests.some(function(recentServiceRequest){
			// 	return recentServiceRequest.args.remarks == resultRequested.args.remarks;
			// });

		 }
    //   if(recentServiceRequests[0].args.remarks != previous_message)
		// 	{
		...
		if(!hasValue){
			...
			console.log('recentServiceRequests',recentServiceRequests);

			sessionStorage.setItem("contracts",JSON.stringify(recentServiceRequests));

		  console.log('json service length->'+recentServiceRequests.length+'\tjson array->'+recentServiceRequests);
		  var htmlString = '<table class="table"><tr></tr><th>Timestamp</th><th>Machine-ID</th><th>MAC Address</th></tr>';
		  for(var i=0; i<recentServiceRequests.length; i++){
		    htmlString = htmlString + '<tr><td>'+ Date(recentServiceRequests[i].args.timestamp)+'</td><td><a href="#" id='+recentServiceRequests[i].args.machineID+' data-machine-id='+recentServiceRequests[i].args.machineID+' onclick="getMachineDetailsById(this)">' + recentServiceRequests[i].args.machineID + '</a></td><td>'+ recentServiceRequests[i].args.remarks + '</td></tr>';
		  }
...
$("#s_timeStamp").val($.now());
	...
	if (typeof web3 !== 'undefined') {
	   ...
	   window.web3 = new Web3(new Web3.providers.HttpProvider("http://192.168.43.212:8545"));
	 //  window.web3 = new Web3(new Web3.providers.HttpProvider("http://10.1.10.85:8545"));
	   console.log("Connectiong to 192.168.43.212- if->"+window.web3);
	  	//window.web3 = new Web3(new Web3.providers.HttpProvider("http://ec2-34-210-156-191.us-west-2.compute.amazonaws.com:8000"));
	  } else {
		 // console.warn("Using web3 detected from external source like AWS")
	   // console.warn("No web3 detected. Falling back to http://192.168.0.104:8545. You should remove this fallback when you deploy live, as it's inherently insecure. Consider switching to Metamask for development. More info here: http://truffleframework.com/tutorials/truffle-and-metamask");
	...
	// window.web3 = new Web3(new Web3.providers.HttpProvider("http://ec2-34-210-156-191.us-west-2.compute.amazonaws.com:8000"));
	  // console.log("Connectiong to remote->"+window.web3);
	  	window.web3 = new Web3(new Web3.providers.HttpProvider("http://192.168.43.212:8545"));
	  //	window.web3 = new Web3(new Web3.providers.HttpProvider("http://10.1.10.85:8545"));
	    console.log("Connected to 192.168.43.212- else->"+window.web3);
	  }
